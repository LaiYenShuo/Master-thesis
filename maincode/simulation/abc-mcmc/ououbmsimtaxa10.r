...
#source("/Users/ms022/Dropbox/TerryLai/R_code/abcmcmc/ououbmabcmcmc.r")
#source("C:/Users/ms022/Dropbox/TerryLai/R_code/abcmcmc/ououbmabcmcmc.r")
...
true.param.array<-c(true.alpha.y,true.alpha.x,true.theta.x,true.sigma.x,true.tau,true.b0,true.b1,true.b2)
#hyper parameters
alpha.y.min <- 0
alpha.y.max <- 0.3
alpha.x.min <- 0
...
for(taxa.size.Index in 1:length(taxa.size.array)){
  ...
  ououbmresult10 <- ououbm_abc_MCMC(startvalue = startvalue, iterations = iterations, y = true.trait$y, x1=true.trait$x1, x2=true.trait$x2, regbound = regbound, sigup = sigup, tree=tree, rootvalue = root, errorbound = errorbound)
